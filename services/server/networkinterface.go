...
func (client NetworkInterfaceClient) AddACGPreparer(ctx context.Context, networkInterfaceNo string, accessControlGroupNoListN string) (*http.Request, error) {
	...
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/addNetworkInterfaceAccessControlGroup")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) AttachPreparer(ctx context.Context, subnetNo string, networkInterfaceNo string, serverInstanceNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/attachNetworkInterface")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) CreatePreparer(ctx context.Context, vpcNo string, subnetNo string, networkInterfaceName string, accessControlGroupNoListN string, serverInstanceNo string, IP string, networkInterfaceDescription string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/createNetworkInterface")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) DeletePreparer(ctx context.Context, networkInterfaceNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/deleteNetworkInterface")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) DetachPreparer(ctx context.Context, subnetNo string, networkInterfaceNo string, serverInstanceNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/detachNetworkInterface")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) GetDetailPreparer(ctx context.Context, networkInterfaceNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("GET", common.GetPath(DefaultBaseURI, "/getNetworkInterfaceDetail")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) GetListPreparer(ctx context.Context, networkInterfaceNoListN string, IP string, networkInterfaceName string, serverInstanceName string, subnetName string, pageNo string, pageSize string, networkInterfaceStatusCode NetworkInterfaceStatusCode) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("GET", common.GetPath(DefaultBaseURI, "/getNetworkInterfaceList")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client NetworkInterfaceClient) RemoveACGPreparer(ctx context.Context, networkInterfaceNo string, accessControlGroupNoListN string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/removeNetworkInterfaceAccessControlGroup")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
