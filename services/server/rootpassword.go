...
func (client RootPasswordClient) GetPreparer(ctx context.Context, serverInstanceNo string, privateKey string) (*http.Request, error) {
	...
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/getRootPassword")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client RootPasswordClient) GetListPreparer(ctx context.Context, rootPasswordServerInstanceListNserverInstanceNo string, rootPasswordServerInstanceListNprivateKey string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/getRootPasswordServerInstanceList")+"?"+common.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
