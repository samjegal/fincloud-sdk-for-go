...
// UpdateSubnet route Table 연관 서브넷 변경
// Parameters:
// routeTableNo - route Table 번호
// parameters - route Table 연관 서브넷 정보
func (client RouteTableClient) UpdateSubnet(ctx context.Context, routeTableNo string, parameters RouteTableUpdateSubnetParameter) (result autorest.Response, err error) {
	if tracing.IsEnabled() {
		ctx = tracing.StartSpan(ctx, fqdn+"/RouteTableClient.UpdateSubnet")
		defer func() {
			sc := -1
			if result.Response != nil {
				sc = result.Response.StatusCode
	...
	req, err := client.UpdateSubnetPreparer(ctx, routeTableNo, parameters)
	if err != nil {
		err = autorest.NewErrorWithError(err, "network.RouteTableClient", "UpdateSubnet", nil, "Failure preparing request")
		return
	}

	resp, err := client.UpdateSubnetSender(req)
	if err != nil {
		result.Response = resp
		err = autorest.NewErrorWithError(err, "network.RouteTableClient", "UpdateSubnet", resp, "Failure sending request")
		return
	}

	result, err = client.UpdateSubnetResponder(resp)
	if err != nil {
		err = autorest.NewErrorWithError(err, "network.RouteTableClient", "UpdateSubnet", resp, "Failure responding to request")
	}

	return
}

// UpdateSubnetPreparer prepares the UpdateSubnet request.
func (client RouteTableClient) UpdateSubnetPreparer(ctx context.Context, routeTableNo string, parameters RouteTableUpdateSubnetParameter) (*http.Request, error) {
	pathParameters := map[string]interface{}{
		"routeTableNo": autorest.Encode("path", routeTableNo),
	}
...
// UpdateSubnetSender sends the UpdateSubnet request. The method will close the
// http.Response Body if it receives an error.
func (client RouteTableClient) UpdateSubnetSender(req *http.Request) (*http.Response, error) {
	sd := autorest.GetSendDecorators(req.Context(), autorest.DoRetryForStatusCodes(client.RetryAttempts, client.RetryDuration, autorest.StatusCodesForRetry...))
	return autorest.SendWithSender(client, req, sd...)
}

// UpdateSubnetResponder handles the response to the UpdateSubnet request. The method always
// closes the http.Response Body.
func (client RouteTableClient) UpdateSubnetResponder(resp *http.Response) (result autorest.Response, err error) {
	err = autorest.Respond(
		resp,
		client.ByInspecting(),
		azure.WithErrorUnlessStatusCode(http.StatusOK),
