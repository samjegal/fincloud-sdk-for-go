...
func (client ServerClient) GetTopPreparer(ctx context.Context, query SeverTargetMetric) (*http.Request, error) {
	...
	signature, err := sec.Signature("POST", common.GetPath(DefaultBaseURI, "/cw_fea/real/cw/api/servers/top")+"?"+common.GetQuery(queryParameters), client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
