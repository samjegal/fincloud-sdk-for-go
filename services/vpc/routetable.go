...
func (client RouteTableClient) CreatePreparer(ctx context.Context, vpcNo string, routeTableName string, supportedSubnetTypeCode SupportedSubnetTypeCode, routeTableDescription string) (*http.Request, error) {
	...
	signature, err := sec.Signature("POST", autorest.GetPath(DefaultBaseURI, "/createRouteTable")+"?"+autorest.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client RouteTableClient) DeletePreparer(ctx context.Context, routeTableNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", autorest.GetPath(DefaultBaseURI, "/deleteRouteTable")+"?"+autorest.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
	preparer := autorest.CreatePreparer(
...
func (client RouteTableClient) GetDetailPreparer(ctx context.Context, routeTableNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("GET", autorest.GetPath(DefaultBaseURI, "/getRouteTableDetail")+"?"+autorest.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
	preparer := autorest.CreatePreparer(
...
func (client RouteTableClient) GetListPreparer(ctx context.Context, routeTableNoListN string, routeTableName string, supportedSubnetTypeCode SupportedSubnetTypeCode, pageNo string, pageSize string, sortedBy string, sortingOrder SortingOrder, vpcNo string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("GET", autorest.GetPath(DefaultBaseURI, "/getRouteTableList")+"?"+autorest.GetQuery(queryParameters), client.Client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
	preparer := autorest.CreatePreparer(
