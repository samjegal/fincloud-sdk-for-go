...
func (client MessageClient) DeletePreparer(ctx context.Context, serviceID string, reserveID string) (*http.Request, error) {
	...
	signature, err := sec.Signature("DELETE", common.GetPathParameters(DefaultBaseURI, "/push/v2/services/{serviceId}/reservations/{reserveId}", pathParameters), client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client MessageClient) GetResultPreparer(ctx context.Context, serviceID string, requestID string) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("GET", common.GetPathParameters(DefaultBaseURI, "/push/v2/services/{serviceId}/messages/{requestId}", pathParameters), client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
...
func (client MessageClient) SendMethodPreparer(ctx context.Context, serviceID string, parameter PushMessageRequestParameter) (*http.Request, error) {
	...
	timestamp := strconv.FormatInt(time.Now().UnixNano()/int64(time.Millisecond), 10)
	sec := security.NewSignature(client.Secretkey, crypto.SHA256)
	signature, err := sec.Signature("POST", common.GetPathParameters(DefaultBaseURI, "/push/v2/services/{serviceId}/messages", pathParameters), client.AccessKey, timestamp)
	if err != nil {
		return nil, err
	}
