		...
		go func(pd string) {
			filepath.Walk(pd, func(path string, info os.FileInfo, err error) error {
				...
				for pn := range packages {
					...
					i := strings.Index(path, "fincloud-sdk-for-go")
					if i == -1 {
						errLog.Fatalf("didn't find 'fincloud-sdk-for-go' in '%s'", path)
					}
...
func writeAliasPackage(ap *AliasPackage, outputPath string, outputLog, errLog *log.Logger) {
	...
	fmt.Fprintln(generatorStampBuilder, `// Licensed under the Apache License, Version 2.0 (the "License");
...
// limitations under the License.`)
	...
	fmt.Fprintln(generatorStampBuilder, "// github.com/samjegal/fincloud-sdk-for-go/tools/profileBuilder")
